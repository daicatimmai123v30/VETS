...
import { CLOSE_CHAT, UNIQUE_CHAT, UNIQUE_ROOM_CHAT,socket,ACTIVE_ROOM, LOAD_MESSAGE, ADD_MESSAGE, CLEAR_MESSAGES} from "../../actions/types";
...
function Chat({location}){
  ...
  const uniqueChat =()=>{
    if(chat.user){
      ...
      {
        const user = unique_Chat.filter(({recieverId})=>recieverId===chat?.user?._id?true:false);
        if(user.length>0)
        {
  ...
  scrollViewChat.current?.scrollIntoView({ behavior: 'smooth' ,block: "end", inline: "nearest"})
  const dispatch =useDispatch();
  return(
      
    <div className="fixed bottom-0 right-0 flex flex-col items-end ml-6 w-full" style={{display:chat.visibleChat?"block":"none",zIndex:1000}}>
      <div className="chat-modal show  mr-5 flex flex-col mb-5 shadow-lg sm:w-1/2 md:w-1/3 lg:w-1/4 view-chat">
        {/* đóng chat */}
        <div className="close-chat bg-red-500 hover:bg-red-600 text-white mb-1 w-10 flex justify-center items-center px-2 py-1 rounded self-end cursor-pointer close" onClick={()=>onCloseChat()} >
        ...
        {/* profile */}
        <div  className="flex justify-between items-center text-white p-2 border shadow-lg mr-5 w-full" style={{backgroundColor:'#E7C7C0',borderWidth:1}}>
          <div className="flex items-center">
            <img src={chat?.user?.image} alt="picture" className="rounded-full w-8 h-8 mr-1"/>
            <h2 className="font-semibold tracking-wider" style={{fontFamily:'Times New Roman'}}>{chat?.user?.firstName}</h2>
          </div>
          {/* chat */}
        </div>
        <div className="flex flex-col bg-gray-200 px-2 chat-services expand overflow-auto  content-chat">
          {
            chat.messages.map((message,index)=>message.senderId===chat.activeUser?(
                <div key={index} className="message p-2 self-end my-2 rounded-md shadow ml-3" style={{backgroundColor:'#E7C7C0',borderWidth:1,color:'#6D5D5D'}}>
                    {message.textMessage}
                </div>):(
                <div key={index} className="chat  p-2 self-start my-2 rounded-md shadow mr-3" style={{backgroundColor:'#F5F3F5',borderWidth:1,color:'#6D5D5D'}}>
                    {message.textMessage}
                </div>
              ))
          }
        ...
        <div className="relative bg-white">
          ...
          <input type="text" placeholder="tin nhắn"
                className=" border border-green-500 focus:outline-none w-full" value={textMessage} onChange={onChangeText} style={{width:'90%'}}/>
                <ion-icon name="send" style={{fontSize:30,color:'gray'}} onClick={sendMessage}></ion-icon>
          </form>
        </div>
      </div>
      <div className="show-chat hidden mx-10 mb-6 mt-4 text-green-500 hover:text-green-600 flex justify-center items-center cursor-pointer ">
        <svg width="4em" height="4em" viewBox="0 0 16 16" className="bi bi-chat-text-fill" fill="currentColor" >
